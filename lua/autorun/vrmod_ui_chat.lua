if CLIENT then
	...
	local chatLog = {}
	local chatPanel = nil
	local nametagSetting = 1
	
	local function addNametags()
		hook.Add( "PostDrawOpaqueRenderables", "vrutil_hook_nametags", function()
			...
			for k,v in pairs(player.GetAll()) do
				...
				cam.Start3D2D( v:GetBoneMatrix(v:LookupBone("ValveBiped.Bip01_Head1")):GetTranslation()+Vector(0,0,15), Angle(0,(g_VR.tracking.hmd.pos - v:GetPos()):Angle().yaw+90,90), 0.1 )
	...
	function VRUtilToggleChat()
		...
		for i = 1,3 do
			...
			button.OnMousePressed = function()
				...
				elseif i == 2 then
					nametagSetting = nametagSetting + 1
					if nametagSetting > 2 then
						nametagSetting = 0
						removeNametags()
					else
						addNametags()
				...
				chatPanel.button2:SetColor(nametagSetting == 0 and Color(255,0,0,255) or nametagSetting == 1 and Color(255,255,0,255) or Color(0,255,0,255))
			end
		end
