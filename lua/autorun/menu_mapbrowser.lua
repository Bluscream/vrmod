if CLIENT then
    ...
    -- I was *so* close to implementing the map browser directly via VRMod's DesktopWindows list, but ran into issues with "window" references.
    -- So uhhh... yeah. More code "borrowed" from Catse.

    local open = false
    
    function VREMapBrowserToggle()
        if !open then
            VREMapBrowserOpen()
        else
            VRUtilMenuClose("vremenu_maps")
        end
    end
    
    
    function VREMapBrowserOpen()
        ...
        local mapCategories = {
            ...
            ["Surf"] = {"surf_"},
            ["The Stalker"] = {"ts_"},
            ["Watching Paint Dry"] = {"hallway", "watchingpaint", "zombiepaint"},
        ...
        function DButton:DoClick()
            if g_VR.active then
                GetConVar("vrutil_autostart"):SetBool(true)
            end
            if (ULib and ULib.ucl.query( LocalPlayer(), "ulx map" ) == true) then
                RunConsoleCommand("ulx","map",selectedMap.name)
            else
        ...
        VRUtilMenuOpen("vremenu_maps", 915, 512, window, mode, pos, ang, 0.03, true, function()
            window:Remove()
            window = nil
            hook.Remove("PreRender","vre_rendermapmenu")
            open = false
        end)
        
        hook.Add("PreRender","vre_rendermapmenu",function()
            ...
            elseif IsValid(vreVRModPanel) then
                ...
                VRUtilMenuRenderPanel("vremenu_maps")
            end
        end)
